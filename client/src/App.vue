<template>
  <v-app style="background: #E3E3EE">
    <!-- Side Navbar -->
    <v-navigation-drawer app tempory fixed v-model="sideNav">
      ...
      <v-card dark v-if="searchResults.length" id="search__card">
        <v-list>
          ...
          >
            ...
            <!-- Show icon if Result is Favorited by User -->
...
export default {
  ...
  watch: {
    user(newValue, oldValue) {
      // if user previously had no value, show snackbar
      if (oldValue === null) {
    ...
    userFavorites(value) {
      // if users favorites value changes
    ...
    sideNavItems() {
      ...
      if (this.user) {
        items = [
          ...
          { icon: "create", title: "Profile", link: "/profile" }
        ];
      }
      return items;
    }
