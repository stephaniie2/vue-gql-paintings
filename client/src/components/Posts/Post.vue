        ...
        <v-card hover>
          <v-card-title>
            ...
            <v-btn @click="handleToggleLike" large icon v-if="user">
              <v-icon large :color="ifPostLiked(getPost._id) ? 'red' : 'grey'">favorite</v-icon>
            </v-btn>
            ...
            <template v-for="message in getPost.messages">
              ...
              <v-list-tile avatar inset :key="message.title">
                ...
                <v-list-tile-action class="hidden-xs-only">
                  <v-icon :color="isOwnMessage(message) ? 'accent' : 'grey'">chat_bubble</v-icon>
                </v-list-tile-action>
              </v-list-tile>
            </template>
          </v-list>
  ...
  data() {
    return {
      ...
      messageRules: [
        ...
        msg => msg.length < 50 || "Message must be less then 50 characters"
      ]
    };
  },
  apollo: {
